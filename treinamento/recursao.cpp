...
string gerarPalindromo(char k) {
	...
	if(k == 'a'){
		...
		string parte_anterior = gerarPalindromo(k - 1);

        string parte_atual;
        parte_atual.push_back(k);
        parte_atual += parte_anterior;
...
bool saquePossivel(int n, int x, int a, int y, int b) {
	if(n == 0){
        ...
        bool variavelX = false, variavelY = false;
        if (x > 0 && n >= a) {
            variavelX = saquePossivel(n - a, x - 1, a, y, b);
        }
        if (y > 0 && n >= b) {
            variavelY = saquePossivel(n - b, x, a, y - 1, b);
        }
...
int main() {
	...
	//Aluno: Gabriel Ribeiro Barbosa da Silva
